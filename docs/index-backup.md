...
<div class="copp-hero">
    ...
    <div class="copp-link-row">
        ...
        <a href="/rust/v0.2.1/copp/">Rust v0.2.1</a>
        <a href="/c/v0.2.0/">C v0.2.0</a>
        <a href="https://pypi.org/project/copp-py/">Python PyPI</a>
    </div>
</div>
...
<div class="copp-grid">
    <div class="copp-card">
        <h3>理解问题</h3>
        <p>从 <a href="#core-problem">核心问题</a> 开始，先看 OPP 怎样从时间域转到路径参数域。</p>
    </div>
    <div class="copp-card">
        <h3>马上试用</h3>
        <p>看 <a href="#installation">安装</a> 和 <a href="#workflow">Step-by-step 工作流</a>；安装和 API 入口支持 Rust/C/Python 同步切换。</p>
    </div>
    <div class="copp-card">
        <h3>选择算法</h3>
...
=== "Rust"
    ...
    `copp` v0.2.1 需要 Rust 1.88 或更新版本。
...
=== "C"
    ...
    动态库、静态库、安装前缀和 smoke test 的细节见 [C v0.2.0 文档](/c/v0.2.0/)。

=== "Python"

    Python binding 适合快速实验、数据处理脚本和与科学计算栈集成。

    ```shell
    pip install copp-py
    ```

    当前 Python 包入口见 [PyPI: copp-py](https://pypi.org/project/copp-py/)，详细 API 示例建议以后随 Python binding 文档一起补齐。


## Step-by-step 工作流 { #workflow }

下面按“建模思想”组织，每一步都可以切换 Rust/C。代码是最小形状，不是完整程序；完整签名、错误处理和生命周期约束请进入对应 API 文档。Python 入口先保留在安装和 API 区域，避免在绑定文档稳定前写入不确定示例。
...
=== "Rust"
    ...
    - [本地 Rust 文档：v0.2.1](/rust/v0.2.1/copp/)
    - [本地 Rust 文档：v0.2.0](/rust/v0.2.0/copp/)
    - [本地 Rust 文档：v0.1.0](/rust/v0.1.0/copp/)
...
=== "Python"
    ...
    - [PyPI: copp-py](https://pypi.org/project/copp-py/)
    - [Python bindings 源码](https://github.com/TOPP-THU/copp/tree/main/bindings/python)

    Python API 文档后续可以作为独立静态目录接入本站，接入方式与 Rust/C API 文档相同。
